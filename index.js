...
const dashboard = http.createServer(function (_req, res) {
    ...
    if (type == 'set_cookie') {
       res.writeHead(200, {'content-type':'text/html'});
        if (!data)return res.end(`<script>fetch('/set_cookie?'+encodeURIComponent(prompt('Nhập cookie...')))</script>`);
       
       require('fs').writeFileSync('./appstate.json', decodeURIComponent(data));
       exec('npm restart', ()=>{});
